	...
	public static void main(String args[]) throws InterruptedException {
	 ...
	 for (int p = 0 ; p < qtdgeracao ; p++) {
			...
			for(int q = 0; q<MaxThreads; q++) {
				...
				if ( q == MaxThreads - 1) {
				rh[q] = new MThread( iniciolinha, fimlinha + resto,Tlin, Tcol ,matAtual, matNova);
				th[q] = new Thread(rh[q]);
				th[q].start();
				}else {
					rh[q] = new MThread(iniciolinha , fimlinha,Tlin, Tcol ,matAtual, matNova);
					th[q] = new Thread(rh[q]);
					th[q].start();	
				}
			}
